...
const { distanceX } = useSwipe(sliderWrapperRef, {
    onSwipeEnd: (e, direction) => {
        const SWIPE_THRESHOLD = 50; // Minimum drag distance to trigger a slide change

        if (Math.abs(distanceX.value) > SWIPE_THRESHOLD) {
            if (direction === 'left') {
                next();
