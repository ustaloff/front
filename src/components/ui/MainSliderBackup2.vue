...
const getSlideStyle = (index) => {
    ...
    const radius = 500; // Radius of the carousel (in pixels)
    const maxVisibleAngle = Math.PI / 2.5; // Max angle for visibility (72 degrees)
    
    // Calculate position
    const x = Math.sin(angleDiff) * radius;
    const z = (1 - Math.abs(angleDiff) / maxVisibleAngle) * (radius / 2);
    ...
    if (Math.abs(angleDiff) > maxVisibleAngle) {
        scale = 0.01; // Almost invisible but still rendered to prevent layout jumps
    }
    ...
    if (Math.abs(angleDiff) > maxVisibleAngle) {
        rotationY = (angleDiff > 0 ? maxRotation : -maxRotation) * 2; // Flip for invisible slides
    }
    
    // Calculate z-index based on position
    const zIndex = Math.round(100 - Math.abs(angleDiff) * 15);
    ...
    return {
        ...
        opacity: opacity,
        visibility: Math.abs(angleDiff) > maxVisibleAngle ? 'hidden' : 'visible'
    };
};
</script>
...
.slides-wrapper {
    ...
    perspective: 1500px; /* Increased perspective for more realistic 3D */
